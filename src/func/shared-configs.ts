...
export function MonitorTabUpdates(targetTexts) {
    const observer = new MutationObserver((mutationsList) => {
        for (const mutation of mutationsList) {
            if (mutation.type === 'childList') {
                for (const addedNode of mutation.addedNodes) {
                    if (addedNode.nodeType === Node.ELEMENT_NODE) {
                        const targetElements = document.querySelectorAll('html[data-light-theme="Savor"] .layout__center .fn__flex-column > .fn__flex > .layout-tab-bar .item:not(.item--readonly).item--focus');
