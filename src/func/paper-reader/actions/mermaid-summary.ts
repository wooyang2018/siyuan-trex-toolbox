...
export async function runMermaidSummary(
    ...
    let response: string;
    try {
        response = await callLLM(config.claudeCliPath, systemPrompt, userPrompt, reporter);
    } catch (e) {
    ...
    if (!mermaidCode) {
        ...
        if (trimmed.startsWith('mindmap') || trimmed.startsWith('flowchart') || trimmed.startsWith('graph')) {
            reporter.log('Claude CLI 返回了无围栏的 Mermaid 代码，尝试使用');
            const blockMd = `\n---\n\n## 📊 知识图谱\n\n\`\`\`mermaid\n${trimmed}\n\`\`\`\n`;
        ...
        return { success: false, message: 'Claude CLI 未返回有效的 Mermaid 代码块' };
    }
